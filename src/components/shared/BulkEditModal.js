...
const NOT_SET_OPTION = { value: NOT_SET, label: 'Clear field' };

function selectValueFor(value) {
  if (value === NOT_SET) return NOT_SET_OPTION;
  ...
  const handleSave = () => {
    ...
    Object.keys(enabled).forEach(k => {
      ...
      if (k.startsWith('custom:')) {
        ...
        updates[k] = fields[k] === NOT_SET ? '' : fields[k];
      }
    });
      ...
      <div className="be-modal animate-fade">
        <div className="be-header">
          <div className="be-title-group">
            ...
            <div>
              <h3>Bulk Edit</h3>
              <p>Updating <strong>{count} record{count > 1 ? 's' : ''}</strong>. Check the fields below to edit them.</p>
            </div>
          </div>
        ...
        <div className="be-body">
          ...
          <div className="be-fields">
            <Field label="Level" fieldKey="level" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.level || [])]}
                value={selectValueFor(fields.level)}
                onChange={opt => set('level', opt?.value || '')} placeholder="Choose level…" {...menuPortalProps} />
            </Field>

            <Field label="GS/WIP/Financial" fieldKey="gsSubmission" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.gsSubmission || [])]}
                value={selectValueFor(fields.gsSubmission)}
                onChange={opt => set('gsSubmission', opt?.value || '')} placeholder="Choose status…" {...menuPortalProps} />
            </Field>

            <Field label="OL Request" fieldKey="olRequest" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.olRequest || [])]}
                value={selectValueFor(fields.olRequest)}
                onChange={opt => set('olRequest', opt?.value || '')} placeholder="Choose status…" {...menuPortalProps} />
            </Field>

            <Field label="OL Received" fieldKey="offerLetter" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.offerLetter || [])]}
                value={selectValueFor(fields.offerLetter)}
                onChange={opt => set('offerLetter', opt?.value || '')} placeholder="Choose status…" {...menuPortalProps} />
            </Field>

            <Field label="Withdraw" fieldKey="withdraw" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.withdraw || [])]}
                value={selectValueFor(fields.withdraw)}
                onChange={opt => set('withdraw', opt?.value || '')} placeholder="Choose…" {...menuPortalProps} />
            </Field>

            <Field label="Payment" fieldKey="payment" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.payment || [])]}
                value={selectValueFor(fields.payment)}
                onChange={opt => set('payment', opt?.value || '')} placeholder="Choose status…" {...menuPortalProps} />
            </Field>

            <Field label="COE/CAS" fieldKey="coeCas" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.coeCas || [])]}
                value={selectValueFor(fields.coeCas)}
                onChange={opt => set('coeCas', opt?.value || '')} placeholder="Choose status…" {...menuPortalProps} />
            </Field>

            <Field label="Savis Fee" fieldKey="savisFee" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.savisFee || [])]}
                value={selectValueFor(fields.savisFee)}
                onChange={opt => set('savisFee', opt?.value || '')} placeholder="Choose…" {...menuPortalProps} />
            </Field>

            <Field label="Refund" fieldKey="refund" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.refund || [])]}
                value={selectValueFor(fields.refund)}
                onChange={opt => set('refund', opt?.value || '')} placeholder="Choose…" {...menuPortalProps} />
            </Field>

            <Field label="Visa Outcome" fieldKey="visaOutcome" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.visaOutcome || [])]}
                value={selectValueFor(fields.visaOutcome)}
                onChange={opt => set('visaOutcome', opt?.value || '')} placeholder="Choose outcome…" {...menuPortalProps} />
            </Field>

            <Field label="Visa Withdraw" fieldKey="visaWithdraw" enabled={enabled} toggle={toggle}>
              <Select className="custom-select" classNamePrefix="react-select" options={[NOT_SET_OPTION, ...toSelectOptions(optionsByField.visaWithdraw || [])]}
                value={selectValueFor(fields.visaWithdraw)}
            ...
            <Field label="Provider Name" fieldKey="providerName" enabled={enabled} toggle={toggle}>
            ...
            <Field label="Course" fieldKey="course" enabled={enabled} toggle={toggle}>
              <input type="text" className="be-input" placeholder="Course name"
                value={fields.course} onChange={e => set('course', e.target.value)} />
            </Field>

            <Field label="Initial Intake" fieldKey="initialIntake" enabled={enabled} toggle={toggle}>
            ...
            <Field label="Other" fieldKey="other" enabled={enabled} toggle={toggle}>
              <input type="text" className="be-input" placeholder="Any other info"
                value={fields.other || ''} onChange={e => set('other', e.target.value)} />
            </Field>

            <Field label="Through" fieldKey="through" enabled={enabled} toggle={toggle}>
              <input type="text" className="be-input" placeholder="Channel / sub-agent"
                value={fields.through || ''} onChange={e => set('through', e.target.value)} />
                        ...
                        {customFields.map(f => (
              <Field key={f.key} label={f.label} fieldKey={`custom:${f.key}`} enabled={enabled} toggle={toggle}>
                {f.type === 'dropdown' ? (
