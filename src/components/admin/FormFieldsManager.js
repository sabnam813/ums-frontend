...
const BUILTIN_SLOTS = {
  'Basic Information': [
    ...
    { id: 'course', label: 'Universities & Courses' },
  ],
  'Intakes': [
    { id: 'deferredIntake', label: 'Initial / Deferred Intake' },
    { id: 'gsSubmission', label: 'GS/WIP/Financial' },
  ],
  'Offer': [
...
export default function FormFieldsManager({ fieldConfig, refetchFields }) {
  ...
  const handleDeleteCustomField = async (field) => {
    if (!window.confirm(
      `Remove the "${field.label}" field?\n\nIt will be moved to trash (admin, under Trash & Backup) and can be restored later if needed.`
    )) return;
    try {
      await axios.delete(`/fields/custom/${field._id}`);
      toast.success(`"${field.label}" field moved to trash`);
      await refetchFields();
    } catch (err) {
  ...
  return (
    <div className="settings-card ffm-card">
      <div className="settings-card-header">
        <h3>Form Fields</h3>
        <p>Add or remove dropdown options, or create entirely new fields. Changes appear instantly on every application form.</p>
      </div>
